...
module.exports = function ({
  ...
  router.post('/scheduled-posts', async (req, res) => {
    ...
    const schedulable = posts
      ...
      .filter(({ post }) => typeof post.image_url === 'string' && post.image_url);
    ...
    try {
      ...
      for (let i = 0; i < schedulable.length; i++) {
        ...
        if (scheduleTimeInput && scheduleTime === null) {
          const err = new Error(`Invalid schedule_time for post ${idx + 1}`);
          err.status = 400;
          throw err;
        }
      ...
      saved.forEach((savedPost) => {
        ...
        if (target) {
          ...
          target.image_url = savedPost.image_url;
        }
      });
      res.status(201).json({
        success: true,
