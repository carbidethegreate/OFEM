(function (global) {
      ...
      sel.innerHTML = fans
        .map(
          (f) =>
            ...
            f.id +
            '">' +
            escapeHtml(f.username || f.name || f.parker_name || String(f.id)) +
            '</option>',
        )
        .join('');
    } catch (err) {
  ...
  async function fetchMessageHistory(fanId, limit) {
    try {
      const res = await global.fetch(
        `/api/messages/history?fanId=${fanId}&limit=${limit}`,
      );
  ...
  function renderMessageHistory(messages) {
    ...
    const items = messages
      .map(
        (m) =>
          `<li><strong>${escapeHtml(m.direction || '')}</strong>: ${escapeHtml(m.body || '')}</li>`,
      )
      .join('');
